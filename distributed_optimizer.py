...
class _DistributedOptimizer(torch.optim.Optimizer):
    def __init__(self, params, named_parameters, compression, is_sparse=False, density=0.001, seq_layernames=None, layerwise_times=None, norm_clip=None, threshold=0, writer=None, gradient_path=None, tb=None, iratio=0.1, stages=1, partitions=0, ec_gradw=1.0, ec_memw=0.0):
        ...
        # self._actual_layerwise_times = []  # list of layerwise actual comm time
        ...
        # #Ahmed - Accumlate dropped grads into memory to apply them later
        # if settings.UPDATE_ITER:
        #     self._grad_memory = {k: torch.zeros_like(v) for k, v in self._merged_parameters.items()}
        #     self._grad_count = 0
        ...
        def hook(*ignore):
            ...
            if not self.local:
                ...
                if new_tensor is not None:
                    ...
                    #     comp_num = ctx.numel()
                    # else:
                    #     comp_num = new_tensor.numel()
                    # self._sum_elems += comp_num
                    # self._avg_elems += comp_num
                    # self._num_sample += 1
    ...
    def synchronize(self):
        ...
        for p, value in self._handles.items():
            ...
            # #Ahmed - add the memory to the reduced grad tensor
            # if self.train_iter and settings.UPDATE_ITER and self.train_iter % settings.UPDATE_ITER == 0:
            #     p.add_(self._grad_memory[name] / self._grad_count)
            #     self._grad_memory[name].zero_()
            #     self._grad_count = 0
        ...
        if rank() == 0 and self.train_iter % settings.DISPLAY == 0 :
            ...
            # self._tb.log('compress/grad_norm_ratio', self._compression.norm)
